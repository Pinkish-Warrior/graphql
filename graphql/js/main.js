...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelector("form").addEventListener("submit", async function (e) {
    ...
    if (!identifier || !password) {
      showError("Identifier and password cannot be empty.");
      return;
    }
  ...
  async function authenticateUser(identifier, password) {
    const base64 = btoa(`${identifier}:${password}`);
    const response = await fetch(endpoint, {
      method: "POST",
      headers: { Authorization: `Basic ${base64}` },
    });

    if (!response.ok) throw new Error("Invalid Identifier/email or password");
  ...
  async function fetchDataAndDisplay(token) {
    try {
      ...
      // Show the elements after fetching data
  ...
  function displayLondonDiv01Projects(data) {
    ...
    displayLineChart(".graphXpAmount", projects.filter(d => d.type === 'xp'), "amount", "#e91e63", "XP Evolution", "#ffffff" );
  }
  ...
  function displayXP(xpData) {
    ...
    displayLineChart(".graphXpAmount", xpData, "amount", "#6420AA", "Cumulative XP Over Time");
  }

  function createBarChart(containerSelector, data, title, yAxisLabel, xAxisLabel, color = "#6420AA") {
  ...
  function displaySkillGraph(transactions) {
    ...
    console.log('Skill Graph Data:', data);
    createBarChart(".graphSkillAmount", data, "Skill GO", "Amount", "Project", "#0d7c66");
  }
  ...
  function AuditRatio(data) {
    ...
    svg.selectAll(".bar")
      ...
      .attr("fill", d => d.type === 'Received' ? '#FF3EA5' : '#73EC8B');
